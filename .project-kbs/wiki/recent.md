...
- **新增 Concept（会话数据模型）** [[0030_sessions-messages-reset]] — sessions/messages 表结构与三个 ID 分工（session_key/session_id/chat_id）、消息逐迭代持久化（rewrite_messages 增量前缀）、中断部分文本带标记落库 + fold 识别标记保持 incomplete、ResetPolicy 空闲/定时重置（换 session_id 不删旧消息、孤儿行生命周期）、_reshape_history 折叠语义、桌面端完整数据流图。

## 2026-09-02

- **新增 Concept（终端面板专门页）** [[0046_shared-session-terminal]] — **共享会话终端**（用户问"terminal 相关有沉淀吗"后确认收录；写页前通读 TerminalPanel.tsx / localPty.ts / _local_tap.py / serve/terminal.py 核实）：**一个通道一个环一个真相源**的三源架构——本地 shell（main 持久 ConPTY，PowerShell 优先，detach 只停流不杀）/ agent 本地命令（`_local_tap.py` 通道注册表：`conv:{session_key}` active 计数并发交错、`proc:{session_key}:{name}` 独占；活进程通道永不逐出=日志唯一副本；atexit+serve 关停树杀防孤儿）/ SSH（ssh_tool 共享注册表 + tap，`/ssh/live/connect` 走 agent 同一 `_ssh_connect`，token 只进内存）；协议要点（数据帧自带结束 offset **客户端 cursor 只被服务端推进**、8K/帧 + 50ms tick、订阅队列只送归属/关闭数据走环防双发、`from` 双向 clamp、本地通道只读 ssh 可输入）；面板行为（conv tab 跟随对话其余钉住、同 key 复活删旧 cursor 重建、连接失败二分 serve 不可达 vs 会话消失、pty 跟随 viewer）；**agentTermTarget 聚焦链**（tool_call 事件 conv_id 匹配活动对话 + 未静音 → conv 立即选 / proc 等首个未见 key、seen 后置防自噬；terminalAutoMuted 手动关面板本轮不再自动弹）；桌面手敲不经审批管线；含排查指引。total_pages 44→45。
- **新增 Change ×3（桌面端工作台批次）** [[0043_desktop-workbench-phase1]] — **桌面端开发工作台 Phase 1**：Monaco 编辑器（气隙引入：ESM + `?worker` + React.lazy，不用 @monaco-editor/react 的 CDN loader）+ chat/workbench 双模式布局（settings.json layoutMode 持久化）+ 编辑器 tab（每 path 一 model 保 undo）+ 外部变更协调（fsVersion 计数器 + dirty 横幅不覆盖草稿）+ diff 可视化（serve `_write_file` 结果内嵌 unified diff ≤5000 字符对齐 patch 形态 + DiffBlock highlight.js 高亮 + 审批卡 args 64K 截断透传「查看变更」）+ 文件树 git 装饰（`git status --porcelain -z` 事件驱动、M/R/C warning·A/U success·D danger + 父目录聚合）+ IDEA 紧凑树（chainOf 单链合并 + 50 层防 symlink 环）+ 运行面板（main/run.ts 每 workdir 一槽 + 历史 ↑ 召回）+ 共享终端三源（本地 pty / agent 本地命令 tap / SSH 共写一通道）。serve 只加字段不改协议。[[0044_desktop-header-workspace-ux]] — **头部交互重构**：按钮与绑定关联规则（操作对象是工作空间内容的跟绑定走——布局隐藏/文件树运行置灰、全局能力不关联）+ 工作空间 chip 删除（store `bindWorkspaceToConv` 随之删）+ **`inWorkbench` 生效布局推导**（未绑定回落 chat，防"按钮藏了退不出工作台"死角）+ 树开关双状态合并（showTree/rightTreeCollapsed 重叠错位）+ 品牌管线（banner→`docs/_make_icons.py`→logo/icon + BrowserWindow icon existsSync 兜底）；踩坑：Fast Refresh 停旧模块致 UI 假回归。[[0045_desktop-git-gutter-marks]] — **编辑器 IDEA 式 git 行标记**：`git:headFile` IPC 三分语义（untracked=空基线全绿 / 非 repo=无标记 / 超 1MiB 对齐读上限）+ `lib/lineDiff.ts`（前后缀裁剪 + LCS Uint32 DP，超 4M cell 降级整块 modified；`''` 按 0 行防幻影配对）+ MonacoPane `headText`（marginClassName 3px 色条走 CSS 变量；**overviewRuler 是 canvas 必须硬编码 hex**）+ 400ms 防抖实时刷新；边界：commit 后已开 tab 标记不自动消。active.md F1 条目订正为已交付。total_pages 41→44。

## 2026-09-01

- **新增 Change + 活页路径大同步** [[0042_package-restructuring]] — **包结构重构**（用户要求按三原则评审并执行：高内聚低耦合 / 单向 DAG 无循环 / 按变化原因分组）：① `serve_resources/` 并入 `gateway/serve/` 按业务分模块（app/chat/admin/browser/terminal/knowledge 各持 `add_routes`，`server.py` 组装）；② 启动器抽出 `src/app/main.py`（原 `cli/app.py`+`__main__.py` 含 gateway/serve 分发，不止 CLI）+ `platforms/` → `gateway/platforms/`（适配器只服务 gateway）+ `cli/` 回归纯 CLI；③ registry 契约下沉 `core/registry.py`（契约放在两侧共依赖的层；`tools/__init__` 只留 re-export + `load_all_tools`，40 个 tool 模块 `from tools import registry` 零改动；core→tools 模块级依赖清零，`core/support/` 零依赖例外）；④ core 21 文件子包化——`agent/`（引擎 7 文件）/`services/`（特性域 4）/`support/`（零依赖 7，DAG 底仅 stdlib）+ 基础设施对流（`tools/_approvals.py`→`core/support/approvals.py`、`_paths.py`→`support/paths.py`、`tool_result_storage.py`→`support/`，消费方是 core 引擎+三模式；tools 40→35）。兼容策略：git mv 保历史、包 `__init__` re-export（`from core.agent import XiheAgent` 7 处零改动）、pyproject glob 天然覆盖。验证：pytest 466 绿 + 全量 py_compile + 分层 import 冒烟 91 工具 + serve 冒烟全 200。**同日活页订正**（本页重构后的路径同步）：concepts/entities 18 页路径更新（`core/agent.py`→`core/agent/agent.py`、`core/prompts.py`→`core/agent/prompts.py`、`core/agent_defs.py`→`core/services/agent_defs.py`、`core/external_agent.py`→`core/services/external_agent.py`、`tools/_approvals.py`→`core/support/approvals.py`、`tools/interrupt.py`→`core/support/interrupt.py`、dispatch 门 `tools/__init__.py:248`→`core/registry.py:272`、`cli/app.py` 按语义分流 launcher→`app/main.py` / SharedContext→`core/context.py`、`src/platforms/`→`src/gateway/platforms/`）+ 0002 分层规则条目重写（import chain 防循环旧表述已失实）+ active.md 活引用修正。**注意：gateway/serve 需重启；桌面端点「重启 xihe」。**total_pages 40→41。

## 2026-08-31

- **新增 Concept（调研沉淀）** [[0041_server-tool-support]] — **Server Tool（服务端内置工具）调研与 xihe 支持路径**（用户要求「把 server tool 的调研结果沉淀到 wiki，注意脱敏」，设计参考/未实施）：`tools` 数组里的**非 function 条目**由模型服务端执行；三家形态对照（Anthropic Messages 版本化类型 / OpenAI 仅 Responses API / 智谱 nested + inline 引用）与两种响应风格（inline 透明 vs 显式块）。**内部网关探针实测矩阵**：`glm-5.2-zp` 组 = litellm Anthropic 通道（chat.completions 上 `web_search_20250305` 形状**真执行**，搜索出公网当天新闻）+ 信息性 tool_call `web_search_prime`（**回合成 tool result 续聊协议已验证**）；现配 `glm-5.2-zp-openai` 组声明层认识（400 校验错误证明 litellm 原样转发）但执行层**全部静默空转**——最危险失败模式。方案设计：`server_tools` config 透传（**非 ToolRegistry 成员**——无 handler/不进 TOOLSETS/不进系统提示词）+ agent.py 两处序列化与判空修复 + 命中名单的 tool_call **不 dispatch 不计迭代直接合成 result**（dangling-repair/compressor/session 不变式全保留）+ 流式复用 tool_call/tool_result 事件零协议改动 + 边界（绕过审批门/按次计费/默认关）。方向判断两车道：对话增强=server tool 终局（商品化），agent 作业=client tool + MCP 结构性留在客户端。开放问题：模型组决策（前置）/ 公网出网边界 / 流式未测。同日 PROTOCOL.md 依据规则新增**脱敏条目**（页面内容必须脱敏：不写密钥/token/凭据，内网地址用 `<内部网关IP>` 占位）。total_pages 盘存校正后 40（lint-status/active 原记 40 为 +1 漂移，本次落盘后实数对齐）。
