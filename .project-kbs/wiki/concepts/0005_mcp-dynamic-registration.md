...
`tools/mcp_tool.py`（核心）/ `core/toolsets.py`（`mcp` toolset，`full` includes `mcp`）/ `core/context.py`（`SharedContext.__init__` 调 `discover_mcp_tools`）/ `config.yaml` / `requirements.txt`（`mcp>=1.2.0,<2` 可选）。
