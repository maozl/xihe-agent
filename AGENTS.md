...
```text
src/core/         Root: config, session, registry, toolsets, context (composition root).
                  agent/ (loop, prompts, compressor, aux client, model catalog),
                  services/ (external_agent, store, agent_defs, diagnostics),
                  support/ (zero-dependency machinery: approvals, interrupt, redact, ...)
src/tools/        Self-registering tool modules and approval interception
src/gateway/      Messaging bot + platforms/ (WeCom, Feishu adapters) + HTTP+WS service (serve/),
                  stream consumer, slash commands
src/cli/          chat.py REPL and doctor.py (CLI mode)
src/app/          The xihe launcher: argparse dispatch into the run modes
...
- `src/core/agent/agent.py`: `XiheAgent`, tool-call loop, streaming, interrupts, approval requests, and context-budget handling.
- `src/app/main.py`: the `xihe` launcher — argparse and subcommand dispatch into the three run modes.
- `src/core/context.py`: `SharedContext`, which reuses SQLite, auxiliary clients, compressor, scheduler, and loaded registry across gateway/serve messages.
...
- `src/core/registry.py`: the tool-registry contract — registration, schema resolution, and dispatch (approval gate, path rewriting). `src/tools/__init__.py` re-exports it and owns `load_all_tools()` auto-discovery.
- `src/gateway/serve/`: HTTP/WS service, one module per business — `app.py` (ServeApp state + system probes), `chat.py` (/stream turn engine + sessions/history), `admin.py` (management panel + specialists + store), `browser.py` (+ `browser_window.py` Win32 snap), `terminal.py` (ssh panel), `knowledge.py` (memory + KBS); `_common.py` + `emitter.py` (worker→WS bridge) are plumbing; `server.py` assembles. Each module owns its `add_routes`.
