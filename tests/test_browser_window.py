"""L0/L1 tests for the browser-panel snap core (gateway/serve/browser_window.py).
...
from gateway.serve import browser_window as bw
...
def test_status_precomputed_running_skips_port_probe(shim, state, monkeypatch):
    # serve probes the port in an executor, then passes the result in — the
    # loop-thread status() call must not re-probe (0.3s of loop stall)
    import tools.browser_tool as bt
    calls = []
    monkeypatch.setattr(bt, "_cdp_port_open", lambda timeout=0.5: calls.append(1))
    shim.add_window(1, pid=100)
    _patch_resolve(monkeypatch)
    res = bw.status(running=True)
    assert res["running"] is True and calls == []


def test_status_precomputed_not_running_skips_port_probe(shim, state, monkeypatch):
    import tools.browser_tool as bt
    calls = []
    monkeypatch.setattr(bt, "_cdp_port_open", lambda timeout=0.5: calls.append(1))
    res = bw.status(running=False)
    assert res["running"] is False and res["hwnd"] is None
    assert calls == []
