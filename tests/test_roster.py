...
class TestAgentDefParsing:
    ...
    def _parse(self, spec, slug="tester"):
        from core.services.agent_defs import _parse_def
        warnings = []
...
class TestXiheAgentRoster:
    """XiheAgent must distinguish [] (base floor only) from None (unrestricted)."""

    def test_empty_list_loads_base_floor_only(self, make_agent):
        from tools import registry
        agent = make_agent(None, enabled_toolsets=[])
        assert agent.enabled_toolsets == {"base"}
        schemas = registry.get_schemas(toolsets=agent.enabled_toolsets,
                                       subagent=True)
        names = {s["function"]["name"] for s in schemas}
        # 只读保底面：有 base 工具、且绝无写面
        from core.toolsets import TOOLSETS
        assert names and names <= set(TOOLSETS["base"]["tools"])
...
def test_roster_prompt_declares_routing_ladder(monkeypatch):
    ...
    monkeypatch.setattr(sat, "_effective_defs", lambda: [fake])
