...
def test_cut_freezes_message_and_continuation_opens_a_new_one():
    # approval flow: pre-approval feed lands on stream A; cut() freezes it
    # (no finish); post-approval events open stream B, which carries the
    # final reply. Nothing after the cut may reuse stream A.
    class _IdAdapter(_FakeAdapter):
        def __init__(self):
            super().__init__()
            self.ids = []
    ...
    async def drive():
        ...
        c.on_tool_start("terminal", '{"command": "dir"}')
        await asyncio.sleep(0.12)      # pre-cut frame delivered
        # from a thread like the tool thread that fires the approval callback;
        # cut() blocks until the old segment's flush has actually been sent
        await asyncio.to_thread(c.cut)
        # ordering barrier: by the time cut() returns (i.e. before the approval
        # card could be sent), the frozen frame is already on the platform
        assert adapter.ids[-1] == adapter.ids[0]
        assert "terminal" in adapter.updates[-1][0]
        c.on_tool_start("write_file", '{"path": "a.txt"}')
        c.on_delta("final answer", kind="content")
        c.finish()
        await asyncio.wait_for(task, timeout=5.0)

    asyncio.run(drive())

    frozen_id = adapter.ids[0]
    assert "terminal" in adapter.updates[0][0]
    assert adapter.updates[0][1] is False          # frozen, not finished
    # cut flushes the pending body once on the old stream, then rotates the id:
    # only that flush may follow the pre-cut update
    assert frozen_id not in adapter.ids[2:], "old stream must stay dead"
    assert "final answer" in adapter.updates[-1][0]
    assert adapter.updates[-1][1] is True          # final finish on the new stream
    assert adapter.ids[-1] != frozen_id
