...
class TestBaseUnion:
    ...
    def test_empty_roster_gains_base_floor(self, make_agent):
        # [] 一律保底 base 只读面——"纯聊天零工具"的旧契约已废除
    ...
    def test_none_unrestricted_also_exposes_base(self, make_agent, static_registry):
        # None = unrestricted：base 工具天然全量暴露——无需（也无法）对
        # None 做 add("base")，哨兵值不是集合，换写成集合反而会把
        # "不限制"塌缩成"只有 base"
        agent = make_agent(None, enabled_toolsets=None)
