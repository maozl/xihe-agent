...
# Touched only on _win_pool's single thread (snap/hide/show/release/status —
# the routes await there); launch()/restart() must not mutate it. That is the
...
# Win32 calls on ANOTHER process's window (ShowWindow / SetWindowPos /
# SetWindowLongPtr, and the hwnd-resolution walks leading into them) block
# until that process pumps messages — a cold-starting or hung Chrome would
# wedge whatever thread calls them. Every entry point therefore runs on this
# one dedicated thread, and the serve routes bound their await so the event
# loop can never be held hostage by Chrome.
_win_pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="browser-win32")
...
def _resolve_hwnd(resurface=True):
    ...
    executor path (launch), never taken here — this runs on _win_pool."""
    if not _IS_WIN:
        return None
    cached = _state["hwnd"]
    ...
    if not wins:
        ...
        # ``resurface=False`` (hide()) skips this — finding it hidden is
        # exactly what hide wants; showing it inside hide is a bug.
        ...
        if resurface:
            ...
            # A bare reveal would show Chrome's remembered geometry (it can
            # dwarf the panel after a monitor change) — re-apply the last
            # snap so the resurface lands in place.
            snapped = _state["snapped"]
            if snapped and _user32.IsWindow(_state["desktop_hwnd"] or 0):
                _user32.SetWindowPos(pick, HWND_TOP, *snapped, SWP_NOACTIVATE)
        _state["hwnd"] = pick
        return pick
...
def status(running=None):
    """`running`: precomputed _cdp_port_open() result, so serve can run the
    0.3s HTTP probe in an executor while the _state-touching body below stays
    on the loop thread."""
    if not _IS_WIN:
        return _unsupported()
...
def hide():
    if not _IS_WIN:
        return _unsupported()
    hwnd = _resolve_hwnd(resurface=False)
    if hwnd is None:
        return {"ok": False, "reason": "no-chrome-window"}
