...
Renders core.services.diagnostics probes as an actionable checklist: every failing
...
from core.services.diagnostics import (readiness_report, probe_optional_deps,
...
def run_doctor(args) -> int:
    ...
    # Registry must be loaded for capability probes. MCP connects in the
    # background; the bounded wait lets a dead server's timeout land in the
    # MCP section of the report instead of racing it.
    from tools import load_all_tools, start_mcp_discovery, wait_for_mcp
    load_all_tools()
    start_mcp_discovery()
    wait_for_mcp(20)
