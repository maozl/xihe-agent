...
def load_memories_checked() -> tuple:
    """Load memories, surfacing corruption instead of hiding it.

    Returns (data, error): a parse failure yields ({}, message) so write
    callers can refuse to clobber a corrupt store ({} + one save would
    silently wipe every memory).
    """
    ...
    try:
        ...
        return {}, f"memories.json 解析失败: {e}"


def _load_memories() -> dict:
    return load_memories_checked()[0]


def _save_memories(data: dict):
    _MEMORIES_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp = _MEMORIES_FILE.parent / (_MEMORIES_FILE.name + ".tmp")
    tmp.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    os.replace(tmp, _MEMORIES_FILE)


def _backup_memories():
    if _MEMORIES_FILE.exists():
        # name + ".bak", not with_suffix — "memories.json".with_suffix() is a trap
        shutil.copy2(_MEMORIES_FILE, _MEMORIES_FILE.parent / (_MEMORIES_FILE.name + ".bak"))
