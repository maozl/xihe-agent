...
def _parse_frontmatter(text: str) -> tuple:
    """Split YAML frontmatter from markdown body → (meta, body); ({}, text) when absent/broken."""
...
_PAGE_DIRS = (("entities", "entity"), ("insights", "insight"), ("concepts", "concept"))


def _list_kbs_pages(root: Path) -> list:
    """Page inventory for the desktop browser. The stem scan is the denominator
    (index titles ≠ file stems routinely, filenames are Chinese); index entries
    join in by stem as enrichment. Tolerant: [] on any failure."""
    try:
        entries, _aliases = _parse_kbs_index(root)
        by_stem = {e["title"].lower(): e for e in entries}
    except Exception:
        by_stem = {}
    pages = []
    for sub, kind in _PAGE_DIRS:
        d = root / "wiki" / sub
        if not d.exists():
            continue
        for p in sorted(d.rglob("*.md")):
            try:
                ...
                continue
            e = by_stem.get(p.stem.lower(), {})
            domain = e.get("domain")
            if not domain and kind == "entity" and p.parent != d:
                domain = p.parent.relative_to(d).parts[0]
            pages.append({
...
def _list_wiki_files(root: Path) -> list:
    """Core wiki files and domain hubs for the browser sidebar."""
    out = []
    for name in ("active.md", "recent.md", "index.md", "log.md"):
        p = root / "wiki" / name
        if p.exists():
            out.append({"rel": p.relative_to(root).as_posix(), "title": p.stem, "kind": "core"})
    d = root / "wiki" / "domains"
    if d.exists():
        for p in sorted(d.glob("*.md")):
            if p.name != "index.md":
                out.append({"rel": p.relative_to(root).as_posix(), "title": p.stem, "kind": "hub"})
    return out


def _list_kbs_candidates(root: Path) -> list:
    """Staging candidates (meta/candidates/*.md, sans index) with frontmatter fields."""
    out = []
    d = root / "meta" / "candidates"
    if not d.exists():
        return out
    for p in sorted(d.glob("*.md")):
        if p.name == "index.md":
            continue
        try:
            meta, _body = _parse_frontmatter(p.read_text(encoding="utf-8", errors="replace"))
        except Exception:
            meta = {}
        dm = meta.get("domain")
        item = {
            ...
            "domains": [dm] if isinstance(dm, str) else (dm if isinstance(dm, list) else []),
        }
        for k in ("related_topic", "next_action", "why_it_matters",
                  "resolved_at", "resolution_target"):
            if meta.get(k):
                item[k] = meta[k]
        out.append(item)
    return out
...
def _list_domains(root: Path) -> list:
    """Domain registry as [{slug, aliases}] — inverse of the alias map."""
    try:
        _entries, aliases = _parse_kbs_index(root)
    except Exception:
        aliases = {}
    inv = {}
    for a, slug in aliases.items():
        inv.setdefault(slug, set()).add(a)
    return [{"slug": s, "aliases": sorted(v - {s})} for s, v in sorted(inv.items())]
